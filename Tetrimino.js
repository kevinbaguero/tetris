class Tetrimino{
    ...
    get estaDentroDelTablero(){
        for (const pmino of this.mapaTablero){
            if (pmino.x < 0) {//Evita salida por izquierda
                return false;
            }
            if (pmino.x >= tablero.columnas){//Evita salida por derecha
                return false;
            }
            if (pmino.y >= tablero.filas){//Evita salida por abajo
                return false;
            }
    ...
    /*
    Esta funcion se encargara del procesamiento logico del dibujado de este objeto
    */
    dibujar(){
        push();
        fill(this.color);
...
function crearMapeoBaseTetriminos(){
    // Muy importante, que no sea let, var ni const de prefijo
    tetriminosBase = {
        Z: {
            color: "red",
            mapa: [
